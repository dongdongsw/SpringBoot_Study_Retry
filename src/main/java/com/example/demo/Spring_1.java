...
public class Spring_1 {
	...
	/*
	 * 객체 지향 프로그래밍이란 무엇인가
	 * 		ㄴ 필요한 데이터를 추상화 시켜 상태와 행위를 가진 객체로 만들고, 객체들간의 상호작용을 통해 로직을 구성하는 프로그램이 방법이다.
	 * 		ㄴ 객체 지향의 특징
     *					ㄴ 추상화란?
	 * 							ㄴ 객체들의 공통된 핵심기능만 드러내고 세부 구현은 감추는 설계기법이다.
	 * 					ㄴ 캡슐화란?
	 * 							ㄴ 데이터와 데이터를 다루는 기능을 하나로 묶고 외부에서 함부로 못건드리게 보호하는 것 이다.
	 * 					ㄴ 상속이란?
	 * 							ㄴ 부모 클래스의 기능을 물려받아서 자식 클래스가 그대로 쓰거나 확장하는 것이다.
	 * 					ㄴ 다형성이란?
	 * 							ㄴ 같은 메서드가 객체의 타입에 따라 다르게 동작하는 것이다.
	 * 
	 * 
	 * 비즈니스 로직이란 무엇인가
